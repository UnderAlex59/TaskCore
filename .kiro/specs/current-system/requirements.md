...
| `REQ-CHAT-003` | Система должна отвечать на task questions через QA Agent, используя current task content, validation result, attachments и cross-task RAG context. | `backend/app/agents/qa_agent_graph.py`, `backend/app/agents/rag_retrieval_graph.py`, `backend/app/services/qdrant_service.py` | `backend/tests/test_chat_agents.py`, `backend/tests/test_rag_retrieval_graph.py` |
| `REQ-CHAT-004` | Система должна извлекать change proposals из чата, обнаруживать duplicate proposals и предоставлять proposal review status. | `backend/app/agents/change_tracker_agent_graph.py`, `backend/app/models/change_proposal.py`, `backend/app/routers/proposals.py` | `backend/tests/test_chat_agents.py` |
...
| `REQ-ADMIN-003` | Система должна поддерживать datasets/runs/exports для RAG Eval, Orchestrator Eval, Adaptation Eval, Validation Eval и QuRE Eval. | `backend/app/services/admin_*_eval_service.py`, `backend/app/models/*_eval.py`, `backend/app/routers/admin.py`, `frontend/src/features/admin/*EvalPage.tsx` | `backend/tests/test_admin_rag_eval_api.py`, `backend/tests/test_admin_orchestrator_eval_api.py`, `backend/tests/test_admin_adaptation_eval_api.py`, `backend/tests/test_admin_validation_eval_api.py`, `backend/tests/test_admin_qure_eval_api.py`, соответствующие frontend `*EvalPage.test.tsx` |
